    ...
    for e in range(1, n_epochs + 1):
        ...
        with torch.no_grad():
            ...
            for step, batch in enumerate(progress_bar):
                ...
                x_hat = generate(model=model, condition=x_0, gen_steps=val_gen_steps)
                ...
                for i in range(B):
                    ...
                    out_clipped = np.empty_like(out_img)
                    for i in range(len(out_img)):
                        lo, hi = gt_img[i].min(), gt_img[i].max()
                        out_clipped[i] = np.clip(out_img[i], lo, hi)

                    val_psnr.append(psnr(gt_img, out_clipped, data_range=1.0))
                    val_ssim.append(ssim(gt_img, out_clipped, data_range=1.0))
        ...
        # print("CT_gt = ", CT_gt.shape)

        if use_standard_norm:
            CT_gt, mean_0, std_0 = standard_normalize(CT_gt)
            CTC_gt, mean_1, std_1 = standard_normalize(CTC_gt)

        CTC_gen = generate(model=model, condition=CT_gt, gen_steps=generation_steps)

        # CTC_gen = torch.clamp(CTC_gen, 0.0, 1.0)
        if use_standard_norm:
        ...
        torchvision.utils.save_image(
            grid,
            f"{IMAGE_OUTPUT_PATH}/{exp_name}_epoch{e+1}.png"
        )
...
def main():
    ...
    # generate_and_save_predictions(model, val_loader, device, output_dir=prediction_dir, generation_f=generate, wandb_run=run)
