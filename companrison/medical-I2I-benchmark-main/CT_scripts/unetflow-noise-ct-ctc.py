...
parser.add_argument('--diff_ckpt', type=str, required=False, default=None,
                    help='Path to the DIFFUSION checkpoint that contains the autoencoder and latent diffusion model.')
        ...
        with torch.no_grad():
            ...
            for step, batch in enumerate(progress_bar):
                ...
                for i in range(B):
                    ...
                    # print("gt stats:", gt_img.min(), gt_img.max(), gt_img.mean())
                    # print("out stats:", out_img.min(), out_img.max(), out_img.mean())

                    val_psnr.append(psnr(gt_img, out_img, data_range=1.0))
                    val_ssim.append(ssim(gt_img, out_img, data_range=1.0))
