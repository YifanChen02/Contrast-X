...
if __name__ == '__main__':
    ...
    data_loader, train_ds     = create_triplet_dataloader(csv_path=args.dataset_csv, resolution=256, 
    ...
    autoencoder, data_loader, autoencoder.encode, autoencoder.decode, autoencoder.encode_multi = \
        accelerator.prepare(autoencoder, data_loader, autoencoder.encode, autoencoder.decode, autoencoder.encode_multi)
    ...
    # with torch.no_grad():
        ...
        for batch in tqdm(data_loader, total=len(data_loader)):
            ...
            zero_matrix      = torch.zeros_like(dce1).to(DEVICE)
            one_matrix       = torch.ones_like(dce1).to(DEVICE)

            clean_inputs          = [dce1, dce2, dce3]
            clean_modalities_mask = [True, True, True]

            patient_id   = batch["DCE1_path"]  # 获取源文件路径

            images = images.to(DEVICE)

            enc_out = autoencoder.encode_multi(clean_inputs, clean_modalities_mask)       
            ...
            zeros          = torch.zeros_like(dce1).to(DEVICE)
 
            images_ct              = [dce1, zeros, dce3]
            broken_modalities_mask = [True, False, True]

            patient_id   = batch["DCE1_path"]  # 获取源文件路径

            images = images.to(DEVICE)

            enc_out    = autoencoder.encode_multi(images_ct, broken_modalities_mask)  
            z_dist     = enc_out.latent_dist                # Normal distribution
            z_mu       = z_dist.mean
            mri_latent = z_mu.cpu().numpy() 
            ...
            for i, latent in enumerate(mri_latent):
                ...
                destpath = out_dir / 'DCE_13.npz'
                np.savez_compressed(destpath, data=latent)
            
            # DCE 1
            images_ct              = [dce1, zeros, zeros]
            broken_modalities_mask = [True, False, False]
            ...
            if ids == 1:
                ...
                use_three_modalities = C >= 3
                ncols = 2 * C   # one column for each modality + its recon

                if use_three_modalities:
                    col_titles = []
                    for k in range(C):
                        col_titles.append(f"DCE{k+1}")
                        col_titles.append(f"DCE{k+1}_recon")
                else:
                    # fallback: 1 modality
                    col_titles = ["Modality", "Modality_recon"]
                ...
                if use_standard_norm:
                    dce1_mean = dce1_mean.cpu().numpy()
                    dce1_std  = dce1_std.cpu().numpy()
                    reconstruction = denormalize(reconstruction, dce1_mean, dce1_std)
                    images         = denormalize(images,         dce1_mean, dce1_std)
                ...
                for i in range(B):
                    ...
                    for c in range(C):
                        axs[i, 2*c].imshow(img_i[c], cmap="gray", vmin=0, vmax=1)   # modality
                        axs[i, 2*c].axis("off")
                        axs[i, 2*c+1].imshow(rec_i[c], cmap="gray", vmin=0, vmax=1) # recon
                        axs[i, 2*c+1].axis("off")

                plt.tight_layout()
                save_path = out_dir / "dce_batch_grid.jpg"
